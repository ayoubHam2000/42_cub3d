	...
	while (sprites[++i])
	{
		...
		if ((sprites[i]->k == 'e' || sprites[i]->k == 'm') && sprites[i]->health > 0)
		{
			...
			while (stripe < drawEndX)
			{
				...
				{
					ft_put_pixel_to_window_img(stripe, drawStartY + HEIGHT / 2 - prog->m_y + 2, 0xff0000);
					ft_put_pixel_to_window_img(stripe, drawStartY + HEIGHT / 2 - prog->m_y + 1, 0xff0000);
		...
		while (stripe < drawEndX)
		{
			texX = (int)(256 * (stripe - (-spriteWidth / 2 + spriteScreenX)) * sprites[i]->tex->w / spriteWidth) / 256;
			if(transformY > 0 && stripe > 0 && stripe < WIDTH && transformY < ZBuffer[stripe])
			{
				y = drawStartY;
				while (y < drawEndY)
				{
					...
					texY = ((d * sprites[i]->tex->h) / spriteHeight) / 256;
					color = ft_get_pixel(sprites[i]->tex, texX, texY);
					if((color & 0x00FFFFFF) != 0)
						ft_put_pixel_to_window_img(stripe, y + HEIGHT / 2 - prog->m_y, tr_c(color, sprites[i]->hit, sprites[i]->k, prog->frame));
					y++;
				}
			}
			stripe++;	
