...
static void	gun_center(void)
{
	...
	while (y < 4)
	{
		...
		while (x < 4)
		{
			ft_put_pixel_to_window_img(x + WIDTH / 2 - 2, y + HEIGHT / 2 - 2, 0xff0000);
			x++;
		}
		y++;
	}
...
static SDL_Surface	*get_gun_tex(t_prog *prog)
{
...
void	draw_gun(t_prog *prog)
{
	...
	while (y < HEIGHT)
	{
		...
		{
			c = ft_get_pixel(tex,
					(x - WIDTH * 0.18) * tex->w / (WIDTH * 0.64),
					(y) * tex->h / (HEIGHT));
			if (c != 0)
				ft_put_pixel_to_window_img(x + WIDTH * 0.0875, y, c);
			x++;
		}
		y++;
	}
	...
	while (y < h)
	{
		...
		while (x < w)
		{
			if (y == 0 || y == h - 1 || x == 0 || x == w - 1)
				ft_put_pixel_to_window_img(x + (WIDTH - w) / 2, y + (HEIGHT - h) - 3, 0);
			else if (x < prog->player.health)
				ft_put_pixel_to_window_img(x + (WIDTH - w) / 2, y + (HEIGHT - h) - 3,
					0x00ff00);
			x++;
		}
		y++;
