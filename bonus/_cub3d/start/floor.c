...
static void	draw_f(t_prog *prog, t_floor *f, int d)
{
	...
	while (++f->y < HEIGHT)
	{
		...
		while (++f->x < WIDTH)
		{
			f->tx = (int)(f->tex->w * (f->floor_x
						- (int)f->floor_x)) & (f->tex->w -1);
			f->ty = (int)(f->tex->h * (f->floor_y
						- (int)f->floor_y)) & (f->tex->h -1);
			f->color = ft_get_pixel(f->tex, f->tx, f->ty);
			f->color = (f->color >> 1) & 8355711;
			if (d)
				ft_put_pixel_to_window_img(f->x, HEIGHT - f->y, f->color);
			else
				ft_put_pixel_to_window_img(f->x, f->y, f->color);
			f->floor_x += f->step_x;
			f->floor_y += f->step_y;
		}
	}
