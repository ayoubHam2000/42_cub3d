...
static void	draw_dir(float angle, float length, t_mini_map *m, float color)
{
	...
	while (r < length)
	{
		x = r * cosf(angle);
		y = r * sinf(angle);
		ft_put_pixel_to_window_img(x + offset_x, y + offset_y, color);
		ft_put_pixel_to_window_img(x + offset_x, y + offset_y + 1, color);
		ft_put_pixel_to_window_img(x + offset_x + 1, y + offset_y, color);
		ft_put_pixel_to_window_img(x + offset_x + 1, y + offset_y + 1, color);
		r += 0.1f;
	}
}
	...
	while (++y < m.y_end)
	{
		...
		while (++x < m.x_end && x / TILE_SIZE < (int)ft_strlen(m.map[m.map_y]))
		{
			...
			if (m.map[m.map_y][m.map_x] == '1')
				ft_put_pixel_in_to_window_img(x + m.shift_x, y + m.shift_y, 0xffffffff);
			else if (ft_in(m.map[m.map_y][m.map_x], "WSEN0emh"))
				ft_put_pixel_in_to_window_img(x + m.shift_x, y + m.shift_y, 100 << 24);
			if (m.map_x == m.player_map_x && m.map_y == m.player_map_y)
				ft_put_pixel_to_window_img(x + m.shift_x, y + m.shift_y, 0xffd700);
			if (m.map[m.map_y][m.map_x] == 'h')
				ft_put_pixel_to_window_img(x + m.shift_x, y + m.shift_y, 0xff00ff);
		}
	}
